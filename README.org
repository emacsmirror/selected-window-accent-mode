...
- Removed obsolete transient dependency from Package-Requires
- Added configurable Pywal color selection (=selected-window-accent-pywal-color=)
- Added robust error handling for Pywal JSON parsing
- Added automatic refresh on customization changes (=:set= handlers)
- Added user feedback messages to toggle functions
- Improved all function docstrings with detailed explanations
- Added future Emacs 31 header-line-active/inactive face support check function
- Fixed minor typos in customization group names

This release focuses on reliability improvements and better user experience, particularly fixing an issue where colored fringes would persist on non-selected windows in tiling mode.
